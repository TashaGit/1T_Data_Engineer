...
def update_table_from_csv():
    for security in securities:
        ...
        try:
            # shutil.copyfile(csv_file, psql_raw_path)  # Копирование файла csv в контейнер с PostgreSQL
            cur.execute(copy_query)
            conn.commit()
    ...
    for security in securities:
        ...
        if last_begin.date() < datetime.strptime(end_date, "%Y-%m-%d").date():
            ...
            if data_shares.shape[0] >= 2:
                data_shares.to_csv(f'./raw_data/last_day_{security}.csv')
                print(f'Файл last_day_{security}.csv сохранен')
                update_table_from_csv() # Обновление таблицы после сохранения файла
            else:
